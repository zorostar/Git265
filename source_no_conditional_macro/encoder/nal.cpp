...
void NALList::serialize(NalUnitType nalUnitType, const Bitstream& bs)
{
    ...
    uint32_t nextSize = m_occupancy + sizeof(startCodePrefix) + 2 + payloadSize + (payloadSize >> 1) + m_extraOccupancy;//m_occupancy + 00 00 00 01 + 16 bit NAL header + payloadSize + payloadSize/2(应对连续出现的00 00) + m_extraOccupancy
