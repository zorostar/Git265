...
private:
    ...
    uint8_t *m_fifo;				//码流
    uint32_t m_byteAlloc;			//m_fifo分配的空间大小
    uint32_t m_byteOccupancy;		//m_fifo已用的空间大小
    uint32_t m_partialByteBits;		//指示m_partialByte中的有效位数
    uint8_t  m_partialByte;			//存储不足一个字节的码流数据，等待填充然后写入m_fifo
