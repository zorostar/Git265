...
int main(int argc, char **argv)
{
    ...
    GetConsoleTitle(orgConsoleTitle, CONSOLE_TITLE_SIZE);  //LZX< 获取窗口标题
    SetThreadExecutionState(ES_CONTINUOUS | ES_SYSTEM_REQUIRED | ES_AWAYMODE_REQUIRED);  //LZX< 保护线程
#endif
    ...
    if (cliopt.csvfn)
    {
        cliopt.csvfpt = x265_csvlog_open(*api, *param, cliopt.csvfn, cliopt.csvLogLevel);//LZX< comma separated value log file 一行存每帧性能
        if (!cliopt.csvfpt)
        {
