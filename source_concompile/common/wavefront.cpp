...
bool WaveFront::init(int numRows)
{
    ...
    m_numWords = (numRows + 31) >> 5;//需要多少个word（32位）才能表示m_numRows
    m_internalDependencyBitmap = X265_MALLOC(uint32_t, m_numWords);
    if (m_internalDependencyBitmap)
        memset((void*)m_internalDependencyBitmap, 0, sizeof(uint32_t) * m_numWords);//初始化为全0

    m_externalDependencyBitmap = X265_MALLOC(uint32_t, m_numWords);
    if (m_externalDependencyBitmap)
        memset((void*)m_externalDependencyBitmap, 0, sizeof(uint32_t) * m_numWords);//初始化为全0
...
{
    uint32_t bit = 1 << (row & 31);//将row装换为00...010...0形式表示，其中1在第row位
    ATOMIC_OR(&m_externalDependencyBitmap[row >> 5], bit);//enable m_externalDependencyBitmap中的row
}
